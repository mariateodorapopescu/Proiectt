...
<head>
    ...
    <!--=============== titlu ===============-->
    <title>Rutare: Acasă → Cel mai apropiat sediu</title>
    ...
    <style>
        ...
        .sediu-item .sediu-name {
            font-weight: bold;
        }
        
        .sediu-item .sediu-distance {
            float: right;
            font-weight: bold;
            color: <%=accent%>;
        }
        
        .sediu-item.active .sediu-distance {
            color: white;
        }
    </style>
</head>
<body>
    ...
    <div class="form-container">
        <h3 style="color: <%=accent%>; margin-top: 0;">Rutare: Acasă → Cel mai apropiat sediu</h3>
        ...
        <div id="allSediiContainer" class="all-sedii-container" style="display: none;">
            <h4 style="margin-top: 0; color: <%=accent%>;">Toate sediile (sortate după distanță)</h4>
            <div id="sediiList"></div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const allSediiContainer = document.getElementById("allSediiContainer");
            const sediiList = document.getElementById("sediiList");
            ...
            require([
                ...
                loadLocationsBtn.addEventListener("click", function() {
                    ...
                    fetch("GetUserToNearestSediuServlet?userId=" + userId)
                        .then(response => {
                            if (!response.ok) {
                        ...
                        .then(data => {
                            ...
                            if (!data.user_location || !data.nearest_sediu) {
                            ...
                            nearestSediu = data.nearest_sediu;
                            allSedii = data.all_sedii;
                            selectedSediu = nearestSediu; // Inițial selectăm cel mai apropiat sediu
                            
                            // Sortăm sediile după distanță
                            allSedii.sort((a, b) => a.distanta_km - b.distanta_km);
                ...
                function displayLocationDetails(data) {
                    ...
                    if (nearestSediu.distanta_km) {
                        distanceBadge.style.display = "inline-block";
                        distanceBadge.textContent = nearestSediu.distanta_km + " km";
                    }
                    
                    updateSediuInfo(nearestSediu);
                }
                
                // Funcție pentru actualizarea informațiilor despre sediul selectat
                function updateSediuInfo(sediu) {
                    // Actualizăm badge-ul cu distanța
                    if (sediu.distanta_km) {
                        distanceBadge.style.display = "inline-block";
                        distanceBadge.textContent = sediu.distanta_km + " km";
                    }
                    
                    // Actualizăm informațiile despre sediu
                    sediuInfo.innerHTML = 
                        `<div class="location-info"><strong>Nume:</strong> ${sediu.nume_sediu}</div>` +
                        `<div class="location-info"><strong>Tip:</strong> ${sediu.tip_sediu}</div>` +
                        `<div class="location-info"><strong>Adresa:</strong> ${sediu.adresa_completa}</div>`;
                    
                    if (sediu.telefon) {
                        sediuInfo.innerHTML += `<div class="location-info"><strong>Telefon:</strong> ${sediu.telefon}</div>`;
                    }
                    
                    if (sediu.email) {
                        sediuInfo.innerHTML += `<div class="location-info"><strong>Email:</strong> ${sediu.email}</div>`;
                    }
                }
                
                // Funcție pentru afișarea listei de sedii
                function displaySediiList() {
                    // Afișăm containerul
                    allSediiContainer.style.display = "block";
                    
                    // Golim lista
                    sediiList.innerHTML = "";
                    
                    // Adăugăm fiecare sediu în listă
                    allSedii.forEach(sediu => {
                        const sediiItem = document.createElement("div");
                        sediiItem.className = "sediu-item";
                        if (sediu.id_sediu === selectedSediu.id_sediu) {
                            sediiItem.className += " active";
                        }
                        
                        sediiItem.innerHTML = 
                            `<span class="sediu-name">${sediu.nume_sediu}</span>` +
                            `<span class="sediu-distance">${sediu.distanta_km} km</span>` +
                            `<div>${sediu.oras}, ${sediu.judet}</div>`;
                        
                        // Adăugăm handler pentru clic
                        sediiItem.addEventListener("click", function() {
                            // Actualizăm sediul selectat
                            selectedSediu = sediu;
                            
                            // Actualizăm informațiile despre sediu
                            updateSediuInfo(sediu);
                            
                            // Marcăm sediul selectat pe hartă
                            highlightSelectedSediu(sediu);
                            
                            // Actualizăm starea vizuală a elementelor din listă
                            document.querySelectorAll(".sediu-item").forEach(item => {
                                item.classList.remove("active");
                            });
                            sediiItem.classList.add("active");
                        });
                        
                        sediiList.appendChild(sediiItem);
                    });
                }
                ...
                function displayLocationsOnMap() {
                    ...
                    // Adăugăm markeri pentru toate sediile
                    allSedii.forEach(sediu => {
                        // Creăm punctul pentru sediu
                        const sediuPoint = new Point({
                            longitude: sediu.longitudine,
                            latitude: sediu.latitudine
                        });
                        
                        // Determinăm dacă acest sediu este cel mai apropiat sau cel selectat
                        const isNearest = sediu.id_sediu === nearestSediu.id_sediu;
                        const isSelected = sediu.id_sediu === selectedSediu.id_sediu;
                        
                        // Creăm marker pentru sediu
                        const sediuGraphic = new Graphic({
                            geometry: sediuPoint,
                            symbol: {
                                type: "simple-marker",
                                color: accentColor, // Culoarea accentului pentru sedii
                                size: isNearest || isSelected ? "15px" : "10px", // Mărim pentru cel mai apropiat/selectat
                                outline: {
                                    color: [255, 255, 255],
                                    width: isNearest || isSelected ? 3 : 1 // Contur mai gros pentru cel mai apropiat/selectat
                                }
                            },
                            attributes: {
                                title: sediu.nume_sediu + (isNearest ? " (Cel mai apropiat)" : ""),
                                description: "Tip: " + sediu.tip_sediu + "<br>Adresa: " + sediu.adresa_completa + 
                                           "<br>Distanța: " + sediu.distanta_km + " km"
                            },
                            popupTemplate: {
                                title: "{title}",
                                content: "{description}"
                    ...
                    // Ajustăm vizualizarea pentru a include locația utilizatorului și cel mai apropiat sediu
                    view.goTo({
                        target: [
                            userPoint, 
                            new Point({
                ...
                function highlightSelectedSediu(sediu) {
                    ...
                    // Afișăm din nou toate locațiile pe hartă
                    // Creăm punctul pentru locația utilizatorului
                    const userPoint = new Point({
                        longitude: userLocation.longitudine,
                        latitude: userLocation.latitudine
                    ...
                    allSedii.forEach(s => {
                        ...
                        // Determinăm dacă acest sediu este cel selectat
                        const isSelected = s.id_sediu === sediu.id_sediu;
                        const isNearest = s.id_sediu === nearestSediu.id_sediu;
                        ...
                        const sediuGraphic = new Graphic({
                            ...
                            symbol: {
                                type: "simple-marker",
                                color: accentColor, // Culoarea accentului pentru sedii
                                size: isSelected ? "15px" : "10px", // Mărim pentru cel selectat
                                outline: {
                                    color: [255, 255, 255],
                                    width: isSelected ? 3 : 1 // Contur mai gros pentru cel selectat
                                }
                            },
                            attributes: {
                                title: s.nume_sediu + (isNearest ? " (Cel mai apropiat)" : ""),
                                description: "Tip: " + s.tip_sediu + "<br>Adresa: " + s.adresa_completa + 
                                           "<br>Distanța: " + s.distanta_km + " km"
                            },
                            popupTemplate: {
                                title: "{title}",
                                content: "{description}"
                    ...
                    // Ajustăm vizualizarea pentru a include locația utilizatorului și sediul selectat
                    view.goTo({
                        target: [
                            userPoint, 
                            new Point({
                ...
                resetBtn.addEventListener("click", function() {
                    ...
                    if (userLocation && allSedii.length > 0) {
                        // Selectăm din nou cel mai apropiat sediu
                        selectedSediu = nearestSediu;
                        
                        // Actualizăm informațiile despre sediu
                        updateSediuInfo(selectedSediu);
                        
                        // Actualizăm starea vizuală a elementelor din listă
                        document.querySelectorAll(".sediu-item").forEach(item => {
                            item.classList.remove("active");
                        });
                        document.querySelectorAll(".sediu-item")[0].classList.add("active");
