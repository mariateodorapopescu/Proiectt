            ...
            try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/test?useSSL=false", "root", "student");
                ...
                if (rs.next()) {
                    ...
                    if (userType == 0) {  // Assuming only type 4 users can approve
                        ...
                        out.println("<h1>Selectati concediul pe care doriti sa il aprobati</h1>");
                        out.print("<form action='");
                        out.print(request.getContextPath());
                        out.println("/aprobsef' method='post'>");  // Assume there's an endpoint to handle this
                        ...
                        String query = "SELECT concedii.id as nr_crt, departament.nume_dep as departament, nume, prenume, " + 
                                ...
                                "NATURAL JOIN tipuri NATURAL JOIN departament JOIN concedii ON concedii.id_ang = useri.id JOIN statusuri ON concedii.status = statusuri.status where concedii.status = 0"; // Assuming status 0 is for pending approval
