...
<%
    ...
    int adeverinteActive = 0, adeverinteTotal = 0;
    ...
    try (Connection connection = DriverManager.getConnection(
        ...
        // ================ EXTRAGERE STATISTICI ADEVERINȚE ================
        String adeverinteQuery = "SELECT " +
                                "COUNT(*) as total, " +
                                "SUM(CASE WHEN status = 0 THEN 1 ELSE 0 END) as in_asteptare " +
                                "FROM adeverinte WHERE id_ang = ?";
        try (PreparedStatement adeverinteStmt = connection.prepareStatement(adeverinteQuery)) {
            adeverinteStmt.setInt(1, id);
            ResultSet adeverinteRs = adeverinteStmt.executeQuery();
            
            if (adeverinteRs.next()) {
                adeverinteTotal = adeverinteRs.getInt("total");
                adeverinteActive = adeverinteRs.getInt("in_asteptare");
            }
        }
...
<head>
    ...
    <title>Gestionare Adeverințe - <%= functie %></title>
...
<body>
    <div class="container">
        ...
        <div class="header fade-in">
            <h1>Gestionare Adeverințe</h1>
            <p class="subtitle">Solicitați și gestionați adeverințele necesare</p>
        </div>
        ...
        <div class="quick-stats fade-in">
            ...
            <div class="stat-card">
                <div class="stat-number"><%= adeverinteTotal %></div>
                <div class="stat-label">Total adeverințe</div>
            </div>
            <div class="stat-card">
                <div class="stat-number"><%= adeverinteActive %></div>
                <div class="stat-label">În procesare</div>
            </div>
            <div class="stat-card">
                <div class="stat-number"><%= (adeverinteTotal - adeverinteActive) %></div>
        ...
        <div class="actions-grid">
            <!-- Cerere adeverință -->
            <a href="addadev.jsp" class="action-card fade-in">
                <i class="ri-file-add-line action-icon"></i>
                <h3 class="action-title">Cerere Adeverință</h3>
                <p class="action-description">
                    Solicitați o nouă adeverință pentru diverse scopuri administrative sau personale.
                </p>
            </a>

            <!-- Vizualizare adeverințe personale -->
            <a href="adeverintenoiuser.jsp?pag=1" class="action-card fade-in" style="animation-delay: 0.1s">
                <i class="ri-file-list-2-line action-icon"></i>
                <h3 class="action-title">Adeverințele Mele</h3>
                <p class="action-description">
                    Vizualizați toate adeverințele solicitate și urmăriți statusul acestora în timp real.
                </p>
            </a>

            <!-- Procesare cereri - doar pentru șefi și directori -->
            <% if (userType == 3 || userType == 0) { %>
            <a href="adeverintenoiuser.jsp" class="action-card fade-in" style="animation-delay: 0.2s">
                <i class="ri-file-check-line action-icon"></i>
                <h3 class="action-title">Procesare Cereri</h3>
                <p class="action-description">
                    Verificați și procesați cererile de adeverințe ale echipei dumneavoastră.
                </p>
            </a>
            <% } else { %>
