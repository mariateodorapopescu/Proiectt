...
<head>
    ...
    <!--=============== titlu ===============-->
    <title>Rutare către departament</title>
...
<body>
    ...
    <div class="form-container">
        <h3 style="color: <%=accent%>; margin-top: 0;">Rutare către departament</h3>
        
        <label for="departamentSelect" style="color: <%=text%>; display: block; margin-bottom: 5px;">Selectează departamentul:</label>
        <select id="departamentSelect">
            <option value="">-- Selectează departamentul --</option>
            <!-- Opțiunile vor fi încărcate dinamic -->
        </select>
        
        <div id="departamentDetails" class="details-panel" style="display: none;">
            <h4>Detalii departament</h4>
            <div id="distanceBadge" class="badge badge-distance" style="display: none;"></div>
            <div id="departamentInfo"></div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const departamentInfo = document.getElementById("departamentInfo");
            ...
            // Funcție pentru încărcarea departamentelor
            function loadDepartamente() {
                // Afișăm indicatorul de încărcare
                loadingSpinner.style.display = "block";
                afișareMesaj('normal', "Se încarcă departamentele...");
                
                fetch("GetDepartamenteServlet")
                    .then(response => {
                        if (!response.ok) {
                            throw new Error("Eroare la încărcarea departamentelor: " + response.statusText);
                        }
                    ...
                    .then(data => {
                        ...
                        // Salvăm datele departamentelor
                        departamente = data;
                        
                        // Completăm dropdown-ul cu departamentele
                        departamente.forEach(dep => {
                        ...
                        afișareMesaj('succes', "Departamentele au fost încărcate.");
                    })
                    .catch(error => {
                        // Ascundem indicatorul de încărcare
                        loadingSpinner.style.display = "none";
            ...
            // Funcție pentru afișarea detaliilor departamentului selectat
            function displayDepartamentDetails(departament) {
                if (!departament) {
                    return;
                }
                
                // Afișăm panoul de detalii
                departamentDetails.style.display = "block";
                
                // Afișăm badge-ul cu distanța dacă este disponibilă
                if (departament.distanta_km) {
                    distanceBadge.style.display = "inline-block";
                    distanceBadge.textContent = departament.distanta_km + " km";
                } else {
                    distanceBadge.style.display = "none";
                }
                
                // Afișăm informațiile despre departament
                departamentInfo.innerHTML = 
                    "<div class=\"departament-info\"><strong>Nume:</strong> " + departament.nume_dep + "</div>" +
                    "<div class=\"departament-info\"><strong>Adresa:</strong> " + departament.adresa_completa + "</div>";
            }
            ...
            require([
                ...
                // Afișăm toate departamentele pe hartă
                function displayAllDepartamente() {
                    // Curățăm graficele existente
                    graphicsLayer.removeAll();
                    
                    // Adăugăm un marker pentru fiecare departament
                    departamente.forEach(dep => {
                        if (dep.latitudine && dep.longitudine) {
                            const depPoint = new Point({
                                longitude: dep.longitudine,
                                latitude: dep.latitudine
                            });
                            ...
                            const depGraphic = new Graphic({
                                ...
                                attributes: {
                                    title: dep.nume_dep,
                                    description: "Adresa: " + dep.adresa_completa
                                },
                                popupTemplate: {
                                    title: "{title}",
                                    content: "{description}"
                    ...
                    if (navigator.geolocation) {
                        ...
                        navigator.geolocation.getCurrentPosition(
                            function(position) {
                                ...
                                // Adăugăm din nou toate departamentele
                                displayAllDepartamente();
                                ...
                                // Actualizăm distanțele pentru departamente
                                updateDepartamentDistances(latitude, longitude);
                                
                                // Dacă există un departament selectat, activăm butonul de generare rută
                                if (selectedDepartament) {
                                    generateRouteBtn.disabled = false;
                                }
                                
                                afișareMesaj('succes', "Localizare reușită. Selectează un departament pentru a genera o rută.");
                            },
                            function(error) {
                ...
                // Handler pentru selectarea departamentului
                departamentSelect.addEventListener("change", function() {
                    const selectedId = this.value;
                    
                    if (!selectedId) {
                        // Nu a fost selectat niciun departament
                        departamentDetails.style.display = "none";
                        selectedDepartament = null;
                        generateRouteBtn.disabled = true;
                        return;
                    }
                    
                    // Găsim departamentul selectat
                    const departament = departamente.find(dep => dep.id_dep == selectedId);
                    
                    if (!departament) {
                        afișareMesaj('eroare', "Departamentul selectat nu a fost găsit.");
                        return;
                    }
                    
                    // Salvăm departamentul selectat
                    selectedDepartament = departament;
                    
                    // Afișăm detaliile departamentului
                    displayDepartamentDetails(departament);
                    
                    // Evidențiem departamentul pe hartă
                    highlightSelectedDepartament(departament);
                    
                    // Activăm butonul de generare rută dacă avem locația utilizatorului
                    if (currentLocation) {
                        generateRouteBtn.disabled = false;
                        afișareMesaj('succes', "Poți genera ruta către departamentul selectat.");
                    } else {
                        afișareMesaj('normal', "Localizează-te întâi pentru a genera ruta.");
                    }
                });
                
                // Funcție pentru evidențierea departamentului selectat pe hartă
                function highlightSelectedDepartament(departament) {
                    if (!departament || !departament.latitudine || !departament.longitudine) {
                        return;
                    }
                    
                    // Găsim și înlocuim markerul departamentului selectat
                    graphicsLayer.graphics.forEach(graphic => {
                        if (graphic.geometry.type === "point" && 
                            graphic.geometry.latitude === departament.latitudine && 
                            graphic.geometry.longitude === departament.longitudine) {
                            // Ștergem markerul vechi
                            graphicsLayer.remove(graphic);
                        }
                    });
                    
                    // Creăm un punct pentru departamentul selectat
                    const depPoint = new Point({
                        longitude: departament.longitudine,
                        latitude: departament.latitudine
                    });
                    
                    // Creăm un marker evidențiat pentru departamentul selectat
                    const depGraphic = new Graphic({
                        geometry: depPoint,
                        symbol: {
                            type: "simple-marker",
                        ...
                        attributes: {
                            title: departament.nume_dep,
                            description: "Adresa: " + departament.adresa_completa
                        },
                        popupTemplate: {
                            title: "{title}",
                            content: "{description}"
                    ...
                    // Zoom și centrare pe departamentul selectat
                    view.goTo({
                        target: depPoint,
                        zoom: 15
                    });
                ...
                // Funcție pentru actualizarea distanțelor departamentelor
                function updateDepartamentDistances(lat, lon) {
                    // Pentru fiecare departament, calculăm distanța față de locația curentă
                    departamente.forEach(dep => {
                        if (dep.latitudine && dep.longitudine) {
                            // Calculăm distanța folosind formula Haversine
                            const R = 6371.0; // Raza Pământului în km
                            const dLat = (dep.latitudine - lat) * Math.PI / 180;
                            const dLon = (dep.longitudine - lon) * Math.PI / 180;
                            const a = Math.sin(dLat/2) * Math.sin(dLat/2) +
                                     Math.cos(lat * Math.PI / 180) * Math.cos(dep.latitudine * Math.PI / 180) * 
                                     Math.sin(dLon/2) * Math.sin(dLon/2);
                            const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1-a));
                            const distance = R * c;
                            
                            // Adăugăm distanța la departament
                            dep.distanta_km = Math.round(distance * 100) / 100;
                        }
                    });
                    
                    // Dacă avem un departament selectat, actualizăm afișarea
                    if (selectedDepartament) {
                        // Actualizăm departamentul selectat cu distanța nouă
                        selectedDepartament = departamente.find(dep => dep.id_dep == selectedDepartament.id_dep);
                        
                        // Afișăm din nou detaliile
                        displayDepartamentDetails(selectedDepartament);
                    }
                }
                ...
                generateRouteBtn.addEventListener("click", async function() {
                    ...
                    if (!selectedDepartament) {
                        afișareMesaj('eroare', "Te rog să selectezi un departament.");
                        return;
                    }
                    ...
                    try {
                        // Creăm un punct pentru locația departamentului
                        const depPoint = new Point({
                            longitude: selectedDepartament.longitudine,
                            latitude: selectedDepartament.latitudine
                        });
                        ...
                        const routeParams = new RouteParameters({
                            stops: new FeatureSet({
                                features: [
                                    ...
                                    new Graphic({ geometry: depPoint }) // Destinația
                                ]
                            }),
                        ...
                        if (routeResult && routeResult.routeResults && routeResult.routeResults.length > 0) {
                            ...
                            if (routeResult.routeResults[0].directions &&
                                ...
                                title.textContent = "Indicații de direcții către " + selectedDepartament.nume_dep;
                ...
                resetBtn.addEventListener("click", function() {
                    ...
                    // Resetăm selectorul de departamente
                    departamentSelect.value = "";
                    
                    // Ascundem detaliile departamentului
                    departamentDetails.style.display = "none";
                ...
                // Afișăm toate departamentele pe hartă inițial
                view.when(() => {
                    window.addEventListener('departamenteLoaded', function() {
                        displayAllDepartamente();
                    });
                });
            });
