        ...
        if (currentUser != null) {
            ...
            try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/test?useSSL=false", "root", "student");
                    ...
                    if (!rs.next()) {
                        ...
                        String userAdresa = rs.getString("adresa");
                        ...
                        if (!isAdmin) {  
                         ...
                         // Obținem informații despre adresa de acasă a utilizatorului
                         String homeAddress = userAdresa;
                         double homeLat = 0;
                         double homeLong = 0;
                         String homeCity = "";
                         String homeCounty = "";
                         ...
                         try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/test?useSSL=false", "root", "student")) {
                             String query = "SELECT strada, latitudine, longitudine, oras, judet, tara " +
                                          "FROM locatii_useri " +
                             ...
                             try (PreparedStatement stmt = con.prepareStatement(query)) {
                                 ...
                                 try (ResultSet rs2 = stmt.executeQuery()) {
                                     if (rs2.next()) {
                                         homeAddress = rs2.getString("strada");
                                         homeLat = rs2.getDouble("latitudine");
                                         homeLong = rs2.getDouble("longitudine");
                                         homeCity = rs2.getString("oras");
                                         homeCounty = rs2.getString("judet");
                                         homeCountry = rs2.getString("tara");
                                         
                                         // Actualizăm adresa completă
                                         homeAddress = homeAddress + ", " + homeCity + ", " + homeCounty + ", " + homeCountry;
                                     }
                                 }
                             }
                         } catch (SQLException e) {
...
<head>
    ...
    <!--=============== titlu ===============-->
    <title>Rutare: Locația actuală → Acasă</title>
...
<body>
    ...
    <div class="form-container">
        <h3 style="color: <%=accent%>; margin-top: 0;">Rutare: Locația actuală → Acasă</h3>
        
        <div class="details-panel">
            <h4>Despre această funcționalitate</h4>
            <p style="margin-top: 0; font-size: 13px;">
                Acest instrument îți permite să vizualizezi ruta de la locația ta actuală către adresa ta de domiciliu.
            </p>
        </div>
        
        <div id="currentLocationDetails" class="details-panel" style="display: none;">
            <h4>Locația mea actuală</h4>
            <div class="badge badge-current">Locație curentă</div>
            <div id="currentLocationInfo">
                <p style="color: <%=text%>; font-style: italic;">Apasă pe butonul "Localizează-mă" pentru a determina poziția ta actuală.</p>
            </div>
        </div>
        
        <div id="homeDetails" class="details-panel">
            <h4>Adresa mea de domiciliu</h4>
            <div class="badge badge-home">Domiciliu</div>
            <div id="homeInfo">
                <% if (homeAddress != null && !homeAddress.isEmpty()) { %>
                    <div class="location-info"><strong>Adresa:</strong> <%= homeAddress %></div>
                <% } else { %>
                    <p style="color: <%=text%>; font-style: italic;">Nu există date despre adresa ta de domiciliu. Contactează administratorul pentru a actualiza informațiile.</p>
                <% } %>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Informații despre adresa de acasă
            const homeAddress = "<%= homeAddress %>";
            const homeLat = <%= homeLat != 0 ? homeLat : "null" %>;
            const homeLong = <%= homeLong != 0 ? homeLong : "null" %>;
            
            // Referințe la elementele DOM
            const currentLocationDetails = document.getElementById("currentLocationDetails");
            ...
            require([
                ...
                // URL pentru serviciul de geocodare și rutare
                ...
                // Verificăm dacă avem coordonatele adresei de acasă
                if (homeLat && homeLong) {
                    // Afișăm adresa de acasă pe hartă
                    displayHomeLocation();
                } else if (homeAddress) {
                    // Geocodificăm adresa de acasă pentru a obține coordonatele
                    geocodeHomeAddress();
                } else {
                    afișareMesaj('eroare', "Nu există informații despre adresa ta de domiciliu. Contactează administratorul pentru a actualiza datele.");
                }
                
                // Funcție pentru afișarea locației de acasă pe hartă
                function displayHomeLocation() {
                    ...
                    const homeGraphic = new Graphic({
                        ...
                        symbol: {
                            type: "simple-marker",
                            color: "#E91E63", // Roz pentru locația de acasă
                            size: "12px",
                            outline: {
                                color: [255, 255, 255],
                                width: 2
                    ...
                    // Adăugăm marker-ul pentru locația de acasă
                    graphicsLayer.add(homeGraphic);
                    
                    // Zoom la locația de acasă
                    view.goTo({
                        target: homePoint,
                        zoom: 14
                    });
                ...
                // Funcție pentru geocodificarea adresei de acasă
                function geocodeHomeAddress() {
                    loadingSpinner.style.display = "block";
                    afișareMesaj('normal', "Se caută coordonatele adresei tale...");
                    
                    const geocodingParams = {
                        address: {
                            "singleLine": homeAddress
                        },
                        outFields: ["Addr_type"]
                    };
                    
                    locator.addressToLocations(locatorUrl, geocodingParams)
                        .then(function(results) {
                            ...
                            if (results.length > 0) {
                                ...
                                // Luăm primul rezultat (cel mai bun scor)
                                const bestMatch = results[0];
                                
                                // Actualizăm coordonatele adresei de acasă
                                homeLat = bestMatch.location.y;
                                homeLong = bestMatch.location.x;
                                
                                // Afișăm adresa de acasă pe hartă
                                displayHomeLocation();
                                
                                afișareMesaj('succes', "Adresa de domiciliu a fost localizată. Apasă pe 'Localizează-mă' pentru a determina poziția ta actuală.");
                            } else {
                                afișareMesaj('eroare', "Nu s-au putut găsi coordonatele pentru adresa ta de domiciliu. Încearcă să te localizezi pentru a genera o rută.");
                            }
                        })
                        .catch(function(error) {
                            loadingSpinner.style.display = "none";
                            console.error("Eroare la geocodificare:", error);
                            afișareMesaj('eroare', "Eroare la căutarea coordonatelor adresei: " + error.message);
                        });
                }
                ...
                locateMeBtn.addEventListener("click", function() {
                    loadingSpinner.style.display = "block";
                    afișareMesaj('normal', "Se determină locația curentă...");
                    ...
                    if (navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(
                            function(position) {
                                ...
                                // Reverse geocoding pentru a obține adresa locației curente
                                ...
                                .then(function(response) {
                                    ...
                                    // Obține adresa locației curente
                                    if (response && response.address) {
                                        currentAddress = response.address.Match_addr || "Adresă necunoscută";
                                    } else {
                                        currentAddress = "Adresă necunoscută";
                                    }
                                    
                                    // Afișăm detaliile locației curente
                                    // currentLocationDetails.style.display = "block";
                                    currentLocationInfo.innerHTML = 
                                        `<div class="location-info"><strong>Adresa aproximativă:</strong> ${currentAddress}</div>` +
                                        `<div class="location-info"><strong>Coordonate:</strong> ${latitude.toFixed(6)}, ${longitude.toFixed(6)}</div>`;
                                    
                                    // Curățăm orice marker anterior al locației curente
                                    ...
                                    // Ajustăm view-ul pentru a include ambele locații
                                    if (homeLat && homeLong) {
                                        const homePoint = new Point({
                                            longitude: homeLong,
                                            latitude: homeLat
                                        ...
                                        view.goTo({
                                            target: [currentLocation, homePoint],
                                            padding: {
                                                top: 50,
                                                left: 350,
                                                right: 50,
                                        ...
                                        afișareMesaj('eroare', "Nu există coordonate pentru adresa de domiciliu. Nu se poate genera ruta.");
                                    }
                                    
                                    afișareMesaj('succes', "Locația curentă a fost determinată cu succes.");
                                })
                                .catch(function(error) {
                                    loadingSpinner.style.display = "none";
                                    console.error("Eroare la obținerea adresei:", error);
                                    
                                    // Afișăm locația curentă fără adresă
                                    currentLocationDetails.style.display = "block";
                                    currentLocationInfo.innerHTML = 
                                        `<div class="location-info"><strong>Adresa:</strong> Indisponibilă</div>` +
                                        `<div class="location-info"><strong>Coordonate:</strong> ${latitude.toFixed(6)}, ${longitude.toFixed(6)}</div>`;
                                    
                                    // Procesăm în continuare locația curentă
                                    // Cod similar cu cel de mai sus...
                                    ...
                                    const pointGraphic = new Graphic({
                                        ...
                                        attributes: {
                                            title: "Locația mea curentă",
                                            description: "Coordonate: " + latitude.toFixed(6) + ", " + longitude.toFixed(6),
                                            isCurrentLocation: true
                                        },
                                        popupTemplate: {
                                            title: "{title}",
                                    ...
                                    afișareMesaj('succes', "Locația curentă a fost determinată cu succes, dar adresa nu a putut fi obținută.");
                                });
                            },
                            function(error) {
                                loadingSpinner.style.display = "none";
                        ...
                        loadingSpinner.style.display = "none";
                        afișareMesaj('eroare', "Geolocalizarea nu este suportată de acest browser.");
                    }
                });
                
                // Handler pentru butonul "Generează rută"
                generateRouteBtn.addEventListener("click", async function() {
                    if (!currentLocation || !homeLat || !homeLong) {
                        afișareMesaj('eroare', "Nu se poate genera ruta. Asigură-te că atât locația curentă cât și adresa de domiciliu sunt disponibile.");
                        return;
                    }
                    ...
                    try {
                        ...
                        const routeParams = new RouteParameters({
                            stops: new FeatureSet({
                                features: [
                                    new Graphic({ geometry: currentLocation }),  // Locația curentă
                        ...
                        if (routeResult && routeResult.routeResults && routeResult.routeResults.length > 0) {
                            ...
                            if (routeResult.routeResults[0].directions &&
                                ...
                                title.textContent = "Indicații de direcții către casă";
                ...
                resetBtn.addEventListener("click", function() {
                    ...
                    view.ui.empty("top-right");
                    currentLocationDetails.style.display = "none";
                    ...
                    // Afișăm din nou locația de acasă dacă avem coordonatele
                    if (homeLat && homeLong) {
                        displayHomeLocation();
                    }
                    
                    // Resetăm mesajul de status
                    afișareMesaj('normal', "Hartă resetată. Apasă pe 'Localizează-mă' pentru a începe.");
                });
                
                // Afișăm un mesaj inițial
                afișareMesaj('normal', "Apasă pe 'Localizează-mă' pentru a determina poziția ta actuală și a genera o rută către domiciliu.");
