...
<head>
    ...
    <!--=============== titlu ===============-->
    <title>Rutare: Sediul Meu → Acasă</title>
...
<body>
    ...
    <div class="form-container">
        <h3 style="color: <%=accent%>; margin-top: 0;">Rutare: Sediul Meu → Acasă</h3>
        ...
        <div id="userHomeDetails" class="details-panel" style="display: none;">
            <h4>Domiciliul meu</h4>
            <div id="homeBadge" class="badge badge-home" style="display: none;">Domiciliu</div>
            <div id="distanceBadge" class="badge badge-distance" style="display: none;"></div>
            <div id="userHomeInfo"></div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const userHomeDetails = document.getElementById("userHomeDetails");
            const userHomeInfo = document.getElementById("userHomeInfo");
            ...
            require([
                ...
                loadLocationsBtn.addEventListener("click", function() {
                    ...
                    fetch("GetSediuToHomeServlet?userId=" + userId)
                        .then(response => {
                            if (!response.ok) {
                        ...
                        .then(data => {
                            ...
                            if (!data.user_sediu_location || !data.user_home_location) {
                            ...
                            userHomeLocation = data.user_home_location;
                        ...
                        .catch(error => {
                            ...
                            if (error.message.includes("coordonatele")) {
                                afișareMesaj('eroare', "Nu s-au găsit coordonatele locației. Verifică dacă adresa de domiciliu are coordonate configurate în sistemul nostru.");
                            ...
                            } else if (error.message.includes("domiciliu") || error.message.includes("acasă")) {
                                afișareMesaj('eroare', "Informațiile despre domiciliul tău nu au putut fi găsite sau nu ai o adresă configurată.");
                            } else {
                ...
                function displayLocationDetails(data) {
                    ...
                    // Afișăm detaliile domiciliului utilizatorului
                    userHomeDetails.style.display = "block";
                    
                    // Afișăm badge-ul de domiciliu
                    homeBadge.style.display = "inline-block";
                    homeBadge.textContent = "Domiciliu";
                    
                    // Afișăm badge-ul cu distanța
                    if (userHomeLocation.distanta_km) {
                        distanceBadge.style.display = "inline-block";
                        distanceBadge.textContent = userHomeLocation.distanta_km + " km";
                    }
                    
                    // Construim informațiile despre domiciliu
                    let homeInfo = `<div class="location-info"><strong>Adresa:</strong> ${userHomeLocation.adresa_completa}</div>`;
                    
                    // Verificăm dacă adresa este aproximativă
                    if (userHomeLocation.adresa_aproximativa) {
                        homeInfo += `<div class="location-info" style="color: #e74c3c;"><strong>Notă:</strong> Aceasta este o locație aproximativă deoarece nu avem coordonatele exacte ale domiciliului tău.</div>`;
                        
                        if (userHomeLocation.adresa_foarte_aproximativa) {
                            homeInfo += `<div class="location-info" style="color: #e74c3c;"><strong>Atenție:</strong> Această locație este doar orientativă și ar putea fi semnificativ diferită de domiciliul tău real.</div>`;
                        }
                    }
                    
                    userHomeInfo.innerHTML = homeInfo;
                    ...
                    console.log("User home location:", userHomeLocation);
                }
                ...
                function displayLocationsOnMap() {
                    ...
                    // Creăm punctul pentru locația domiciliului utilizatorului
                    const userHomePoint = new Point({
                        longitude: userHomeLocation.longitudine,
                        latitude: userHomeLocation.latitudine
                    });
                    
                    // Creăm marker pentru locația domiciliului utilizatorului
                    const userHomeGraphic = new Graphic({
                        geometry: userHomePoint,
                        symbol: {
                            type: "simple-marker",
                            color: "#e91e63", // Roz pentru domiciliu
                            size: "12px",
                            outline: {
                                color: [255, 255, 255],
                                width: 2
                        ...
                        attributes: {
                            ...
                            description: userHomeLocation.adresa_completa
                        },
                        popupTemplate: {
                            title: "{title}",
                            content: "{description}"
                    ...
                    // Adăugăm marker-ul pentru domiciliul utilizatorului
                    graphicsLayer.add(userHomeGraphic);
                    
                    // Ajustăm vizualizarea pentru a include ambele locații
                    view.goTo({
                        target: [userSediuPoint, userHomePoint],
                        padding: {
                            top: 100,
                            bottom: 100,
                            left: 350,
                ...
                generateRouteBtn.addEventListener("click", async function() {
                    if (!userSediuLocation || !userHomeLocation) {
                    ...
                    try {
                        // Creăm punctele pentru locațiile sediului și domiciliului utilizatorului
                        const userSediuPoint = new Point({
                            longitude: userSediuLocation.longitudine,
                            latitude: userSediuLocation.latitudine
                        });
                        ...
                        const routeParams = new RouteParameters({
                            stops: new FeatureSet({
                                features: [
                                    ...
                                    new Graphic({ geometry: userHomePoint })    // Locația domiciliului utilizatorului
                                ]
                            }),
                        ...
                        if (routeResult && routeResult.routeResults && routeResult.routeResults.length > 0) {
                            ...
                            if (routeResult.routeResults[0].directions &&
                                ...
                                title.textContent = "Indicații de direcții către domiciliu";
                                ...
                                googleMapsLink.href = `https://www.google.com/maps/dir/${userSediuLocation.latitudine},${userSediuLocation.longitudine}/${userHomeLocation.latitudine},${userHomeLocation.longitudine}`;
