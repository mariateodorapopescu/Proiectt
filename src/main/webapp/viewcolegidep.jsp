    ...
    if ("true".equals(request.getParameter("json"))) {
        ...
        try (Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/test?useSSL=false", "root", "student");
             PreparedStatement stmt = connection.prepareStatement(
                 "SELECT useri.nume, useri.prenume, tipuri.denumire AS functie, departament.nume_dep AS departament FROM useri " +
                 "LEFT JOIN tipuri ON useri.tip = tipuri.tip " +
                 "LEFT JOIN departament ON departament.id_dep = useri.id_dep " +
                 "WHERE useri.id_dep = ? AND username <> 'test'")) {
        ...
        responseJson.put("header", "Angajati din departamentul meu");
    ...
    <body style="--bg:<%out.println(accent);%>; --clr:<%out.println(clr);%>; --sd:<%out.println(sidebar);%>">
            ...
            <div style=" border-radius: 2rem;" class="content">
                <div class="intro" style=" border-radius:2rem; background:<%out.println(sidebar);%>; color:<%out.println(text);%>">
                    <div class="events"  style="background:<%out.println(sidebar);%>; color:<%out.println(text);%>" id="content">
                        ...
                        <h1 id="tableTitle">Angajati din departamentul meu</h1>
                        <h3 id="tableDate"></h3>
     ...
     <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            fetch("viewcolegidep.jsp?json=true")
                .then(response => {
                    if (!response.ok) {
                ...
                .then(jsonData => {
                    ...
                    if (!jsonData || !jsonData.data || jsonData.data.length === 0) {
                        ...
                        document.getElementById("tableTitle").textContent = "Nu există angajați disponibili!";
                        return;
                    }
...
async function sendJsonToPDFServer() {
    ...
    try {
        let response = await fetch("viewcolegidep.jsp?json=true");
